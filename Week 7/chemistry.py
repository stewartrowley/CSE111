...
def main():
    ...
    simplified = parse_formula(molecular_formula, get_periodic_table)


    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.

    molar_mass = compute_molar_mass(simplified, get_periodic_table)
    
    atoms = get_atoms(molar_mass, mass_in_grams)
...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    for symbol, quantity in symbol_quantity_list:
        ...
        partial_molar_mass = quantity * atomic_mass
        molar_mass += partial_molar_mass
    
    return molar_mass

def get_atoms(molar_mass, mass_in_grams):
    mol = mass_in_grams / molar_mass
    number_of_atoms = mol * 6.022 * 10 ** 23
